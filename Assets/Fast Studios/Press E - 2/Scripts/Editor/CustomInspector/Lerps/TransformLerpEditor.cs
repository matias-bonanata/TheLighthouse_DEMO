...
{
    [CustomEditor(typeof(TransformLerp))]
    public class TransformLerpEditor : Editor
    {
        private VisualElement _root;
        public VisualTreeAsset visualTree;
        public StyleSheet USS;
        TransformLerp localTarget;
        private LerpTimelinePreviewUI _timeline;

        private const int TransformLerpTabIndex = 100;
        private const int LerpTabIndex = 101;
        private int selectedTab = TransformLerpTabIndex;
        ...
        VisualElement TransformLerpTab;
        VisualElement LerpTab;

        void LoadLastSession(TransformLerp lTarget)
        {
            if (lTarget.lastSelectedTab >= 5 && lTarget.lastSelectedTab <= 8)
                lTarget.lastSelectedTab = TransformLerpTabIndex + (lTarget.lastSelectedTab - 5);

            if (lTarget.lastSelectedTab < TransformLerpTabIndex)
                lTarget.lastSelectedTab = TransformLerpTabIndex;
        ...
        {
            localTarget = target as TransformLerp;
            ...
            TransformLerpTab = Lower.Q<VisualElement>(name: "TransformTab");
            LerpTab = Lower.Q<VisualElement>(name: "LerpTab");

            root.Bind(serializedObject);

            Button TransformButton = TabsParent.Q<Button>(name = "TransformButton");
            Button LerpButton = TabsParent.Q<Button>(name = "LerpButton");
            Button[] allButtons = { TransformButton, LerpButton };
            ...
            SetupPageChange(TransformButton, TransformLerpTabIndex, allButtons);
            ...
            // Show Ifs
            {
                ...
                VisualElement PosCurve = root.Q<VisualElement>(name = "PosCurve");
                VisualElement RotCurve = root.Q<VisualElement>(name = "RotCurve");
                VisualElement ScaleCurve = root.Q<VisualElement>(name = "ScaleCurve");

                FSEditorUI.ShowIfBool(root, serializedObject, nameof(localTarget.overridePositionCurve), PosCurve);
                FSEditorUI.ShowIfBool(root, serializedObject, nameof(localTarget.overrideRotationCurve), RotCurve);
                FSEditorUI.ShowIfBool(root, serializedObject, nameof(localTarget.overrideScaleCurve), ScaleCurve);
        ...
        void SetTabIndex(int i, Button[] allButtons)
        {
            ...
            int index = i == TransformLerpTabIndex ? 0 : 1;
        ...
        {
            if (TransformLerpTab != null && LerpTab != null)
            {
                FSEditorUI.SetVisible(selectedTab == TransformLerpTabIndex, HiddenClass, TransformLerpTab);
