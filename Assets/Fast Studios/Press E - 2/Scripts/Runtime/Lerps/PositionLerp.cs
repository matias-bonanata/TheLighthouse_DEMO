...
{
    public class PositionLerp : PressELerps
    {
        public Vector3 NewPos;

        public override void Interact() => InteractPosition(NewPos);
        ...
        {
            Vector3 targetBinding = NewPos;
            Vector3 startBinding;

            if (LerpType == LerpType.Add)
            {
                if (ToggleWithOriginalState)
                {
                    Vector3 origWorld = PeekOriginalWorldOrCurrent(tr, t => t.position);
                    Vector3 baseBinding = PositionWorldToBinding(tr, origWorld);
                    targetBinding = baseBinding + NewPos;
                }
                else
                {
                    startBinding = (WhatIsGoingToAffect == TransformApplyType.World) ? tr.position : tr.localPosition;
                    targetBinding = startBinding + NewPos;
                }
            }

            startBinding = (WhatIsGoingToAffect == TransformApplyType.World) ? tr.position : tr.localPosition;
            Vector3 targetWorld = PositionBindingToWorld(tr, targetBinding);

            float u = __Editor_GetTimelineU();
            float k = __Editor_EvalBaseCurve(u);

            Vector3 previewBinding = Vector3.Lerp(startBinding, targetBinding, k);
            Vector3 previewWorld = PositionBindingToWorld(tr, previewBinding);

            DrawPreviewModel(tr, previewWorld, tr.rotation, tr.lossyScale);

            DrawPreviewDottedLine(tr.position, targetWorld);
        }

        [MenuItem("CONTEXT/PositionLerp/Open Documentation", false)]
        static void OpenDoc()
        {
